...
if ($query === '') {
    // No query provided: display the full list of superheroes
    echo "<h2>All Superheroes</h2>";
    echo "<ul>";
    ...
    // Search for the query in superheroes
    $found = false;
    foreach ($superheroes as $superhero) {
        if (strcasecmp($query, $superhero['alias']) === 0 || strcasecmp($query, $superhero['name']) === 0) {
            // Match found
            echo "<h2>Superhero Found</h2>";
            echo "<h3>Alias: " . htmlspecialchars($superhero['alias'], ENT_QUOTES, 'UTF-8') . "</h3>";
            echo "<h4>Name: " . htmlspecialchars($superhero['name'], ENT_QUOTES, 'UTF-8') . "</h4>";
            echo "<p>Biography: " . htmlspecialchars($superhero['biography'], ENT_QUOTES, 'UTF-8') . "</p>";
            $found = true;
            break;
        }
    }
    ...
    if (!$found) {
        ...
        echo "<p>No superhero matches your query.</p>";
    }
}
?>
